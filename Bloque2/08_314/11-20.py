...
class AppLimites(ctk.CTk):
    def __init__(self):
        ...
        self.pan_x = 0  # Desplazamiento de vista
        self.pan_y = 0  # Desplazamiento de vista
    ...
    def ejecutar_analisis(self):
        try:
            ...
            # Resetear desplazamiento cuando se calcula un nuevo límite
            self.pan_x = 0
            self.pan_y = 0

            # Calcular límite usando algoritmo propio
            ...
            # Graficar con el límite calculado
    ...
    # ====== FUNCIONES AUXILIARES PARA CÁLCULO DE LÍMITES ======
    ...
    def detectar_limite_trigonometrico(self, f, x_sym, h_val):
        ...
        # Casos notables: sin(x)/x, tan(x)/x, (1-cos(x))/x²
        try:
            # sin(ax)/sin(bx) → a/b
            if 'sin' in f_str and f_str.count('sin') == 2:
                ...
                if denom.has(sp.sin):
                    # Extraer argumentos
                    sin_numer = sp.sin(x_sym)  # placeholder
    ...
    def racionalizar_correctamente(self, f, x_sym, h_val, numer, denom):
        ...
        try:
            ...
            # Multiplicar por conjugado si hay raíz
            if sqrt_terms_numer or sqrt_terms_denom:
                # Estrategia: si √(a+b) - √a, multiplicar por √(a+b) + √a
                # Detectar patrón específico
                
                # Caso simple: (√(x+a) - √a) / x, multiplicar por (√(x+a) + √a) / (√(x+a) + √a)
    ...
    def analizar_limites_laterales_correctamente(self, f, x_sym, h_num):
        ...
        # Usar epsilones progresivamente más pequeños
        ...
        # Verificar convergencia usando solo los más cercanos
        left_limit = None
        right_limit = None
        
        if left_values and len(left_values) >= 2:
            # Usar los dos últimos (más cercanos)
            eps1, val1 = left_values[-2]
            eps2, val2 = left_values[-1]
            
            if abs(val1 - val2) < 1e-5:  # Convergencia
                left_limit = val2
        ...
        # Determinar conclusión sobre límite bilateral
    ...
    def calcular_infinito_correcto(self, grado_numer, grado_denom, coef_numer, coef_denom, h_val):
        ...
        # Signo del coeficiente principal
        sig_coef = 1 if coef_numer * coef_denom > 0 else -1
        
        if diferencia % 2 == 0:
            # Diferencia PAR: siempre +∞
            return sp.oo
        else:
            # Diferencia IMPAR: signo depende de h_val
            if h_val == sp.oo:
    ...
    def graficar(self, f, x_sym, h_val):
        ...
        try:
            ...
            # Detectar discontinuidades y quebrar líneas
            # Para evitar que Matplotlib una puntos a través de asíntotas
            discontinuities = []
            threshold = 50  # Si el salto > 50 unidades, hay discontinuidad
            ...
            # Plotear segmentos de forma separada
            if discontinuities:
                start_idx = 0
                for disc_idx in discontinuities:
                    # Plotear hasta la discontinuidad
                ...
                # Sin discontinuidades, plotear todo
                self.ax.plot(x_vals, y_vals, color=PRIMARY_COLOR, linewidth=2.5, 
                            label="f(x)", zorder=3)
            
            # Filtrar valores válidos para límites de Y
            ...
            # Aplicar desplazamiento vertical (solo a la vista)
        ...
        try:
            ...
            if h_val == sp.oo or h_val == -sp.oo:
                ...
                if grado_numer < grado_denom:
                    ...
                    # Calcular infinito con signo correcto
            ...
            # ==== PASO 3: Detectar Límites Trigonométricos Notables ====
            ...
            # ==== PASO 4: Detección de Indeterminación 0/0 ====
            ...
            if numer_val == 0 and denom_val == 0:
                ...
                # ---- PASO 4b: Racionalización Correcta ----
                if f.has(sp.sqrt):
                ...
                # ---- PASO 4c: Serie de Taylor (como verificación) ----
            ...
            # ==== PASO 5: Simplificación Alternativa ====
            ...
            # ==== PASO 6: Análisis de Límites Laterales ====
